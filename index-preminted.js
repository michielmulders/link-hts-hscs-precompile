...
async function main() {
  ...
  let mintTxRx = await mintTxSubmit.getReceipt(client);
  
  console.log(`- Created NFT ${tokenId} with serial: ${mintTxRx.serials[0].toString()}`);
  ...
  const contractExecTx = await new ContractExecuteTransaction()
    ...
    .setGas(3000000)
    .setFunction(
      "mintNonFungibleToken", // Call correct function name
      new ContractFunctionParameters().addUint64(1).addBytesArray([Buffer.from("m")]) // Mint 1 NFT
    );
  ...
  const contractExecTx2 = await new ContractExecuteTransaction()
    ...
    .setFunction(
      ...
      new ContractFunctionParameters()
        ...
        .addInt64(mintTxRx.serials[0].low) // or 1 normally
    )
    .freezeWith(client);
